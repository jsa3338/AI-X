...
SELECT * 
    FROM DEPT;  -- 4행
-- CROSS JOIN  테이블행 각각에 다른테이블 행이 붙어버리는 결과. 즉 의미가 없다
SELECT * 
    FROM EMP, DEPT  
    WHERE  ENAME ='SCOTT' ;
...
SELECT ENAME, JOB, EMP.DEPTNO, DNAME
    FROM EMP, DEPT
    WHERE ENAME ='SCOTT' AND EMP.DEPTNO = DEPT.DEPTNO;
SELECT EMPNO NO, ENAME, JOB, MGR, HIREDATE, SAL, COMM, E.DEPTNO, DNAME
    FROM EMP E, DEPT D
    WHERE E.DEPTNO = D.DEPTNO
    ORDER BY E.DEPTNO, NO;
    --EX. 82년도에 입사한 10, 20번 부서직원의 이름 , 급여, 근무지 (급여순 출력)
SELECT ENAME,  SAL, LOC
    FROM EMP E, DEPT D
    WHERE E. DEPTNO = D.DEPTNO AND TO_CHAR(HIREDATE, 'RR') = '82' AND E.DEPTNO IN (10, 20)
    ORDER BY SAL;
    -- EX. JOB이 'SALESMAN'이거나 'MANAGER'인 사원의
        -- 이름, 급여, 상여, 연봉(SAL_COMM)*12, 부서명, 근무지(연봉이 큰 순) 출력
    SELECT ENAME, SAL, COMM, (SAL+NVL(COMM,0))*12 ANNUALSAL, LOC
        FROM EMP E, DEPT D
        WHERE E.DEPTNO = D.DEPTNO
        ORDER BY ANNUALSAL DESC;
    --EX. COMM 이 NULL 이고 SAL이 2000대인 사원의 이름 급여 입사일 부서번호 부서명(부서명, 급여순 정렬)
...
SELECT EMPNO, ENAME, JOB, MGR, SAL, GRADE || '등급' AS GRADE
    ...
    -- EX. 모든 사원의 사번, 이름, 직책, 상사사번, 급여, 급여등급(1등급, 2등급..), 부서명
SELECT EMPNO, ENAME, JOB, MGR, SAL, GRADE || '등급' AS GRADE, DNAME
...
SELECT WORKER.EMPNO, WORKER.ENAME, WORKER.MGR, MANAGER.EMPNO, MANAGER.ENAME MGRNAME
...
-- EX.모든 사원의 사번, 이름, 상사의 사번, 상사 이름
SELECT W.EMPNO, W.ENAME, W.MGR, M.ENAME MGRNAME
    FROM EMP W, EMP M
    WHERE W.MGR=M.EMPNO;
-- EX. SMITH의 상사는 JONES 이다 포맷으로 출력하시오
SELECT W.ENAME || '의 상사는 ' || NVL(M.ENAME, '없음') ||'이다.' MESSAGE
    FROM EMP W, EMP M
    WHERE W.MGR = M.EMPNO(+);
...
----------------------------
-- 4. OUTER JOIN -- SELF JOIN EQUI JOIN 시 조건이 만족하지 않는 행까지 나타게 하는 JOIN
----------------------------
-- 배제된 행을 결과에 포함시키려는 경우 (+) 기호를 정보가 부족한 칼럼 이름 뒤에 덧붙임
--(1) SELF JOIN 에서의 OUTER JOIN
SELECT W.EMPNO, W.ENAME, W.MGR, M.EMPNO, M.ENAME MGRNAME
    FROM EMP W, EMP M
    WHERE W.MGR=M.EMPNO(+);
...
--8. 이름, 급여, 등급, 부서명, 연봉, 직속상사명. 연봉=(급여+comm)*12으로 계산
    SELECT W.ENAME, W.SAL, GRADE, DNAME, (W.SAL+W.COMM)*12 연봉, M.ENAME
        FROM EMP W, EMP M, DEPT D, SALGRADE
        WHERE W.DEPTNO = D.DEPTNO AND W.MGR=M.EMPNO(+) AND W.SAL BETWEEN LOSAL AND HISAL
        ORDER BY DNAME;
...
--11. 사원명, 상사명, 상사의 상사명을 검색하시오(self join)
    SELECT W.ENAME, M.ENAME 상사명, HM.ENAME 차상위자
