...
--  ※ DCL ※ --
-- --------- --
create user jsa3338 identified by 'mysql'; -- 계정생성
grant all privileges on *.* to jsa3338; -- dba 권한부여
revoke all privileges on *.* from jsa3338; -- 권한 박탈
...
truncate personal;
load data local infile 'd:/mega_IT/personal.txt' replace into table personal; -- MySql 5.x에서 가능
...
select p1.pno, p1.pname, p2.pname manager
	from personal p1 right join personal p2
...
select p1.pno, p1.pname, ifnull(p2.pname, '★CEO★') manager, d.dname
	from personal p1  left outer join personal p2 
    on p1.manager = p2.pno, division d
    where p1.dno = d.dno;
...
-- date_format(문자,     포맷) => 문자형을 날짜형으로
    ...
    -- %d(일01,02,.) %e(일1,2,..) %H(24시간) %h(12시간) %p(오전,오후) %i(분) %s(초)
