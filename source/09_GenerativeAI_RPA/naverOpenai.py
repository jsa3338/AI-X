...
def get_naver_api_data(media, word):
    "word에 대해 media 검색한 결과의 str을 리턴"
    load_dotenv()

    client_id = os.getenv('Client_ID')
    client_secret = os.getenv('Client_Secret')
    encText = urllib.parse.quote(word)
    url = f"https://openapi.naver.com/v1/search/{media}?display=20&sort=date&query={encText}"
...
def get_openai_shopping_analysis(wb):
    # 2. prev_list, now_list 전체 내용 불러오기
...
def get_openai_news_summarize(str_news_data):
    # 뉴스 요약 리턴
    prompt = f'''다음 뉴스 내용을 구체적인 수치, 고유 명사를 언급하며 글머리를 활용하여
    한글 200자 이내로 요약글을 작성해줘.
    뉴스 내용 {str_news_data}'''
    return aiconn(prompt=prompt)